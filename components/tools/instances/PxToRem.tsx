...
export default function PxToRem() {
  ...
  useEffect(() => {
    const val = parseFloat(px) / parseFloat(base)
    setRem(val.toFixed(3))
  }, [px, base])
    ...
    <div className="space-y-8">
      <div className="bg-white dark:bg-[#0D1526] border border-gray-100 dark:border-[#1E2D47] rounded-3xl p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h3 className="text-sm font-black uppercase tracking-widest text-gray-900 dark:text-white">PX to REM Converter</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-1">Pixels (PX)</label>
              <input
                type="number"
                value={px}
          ...
          <div className="flex flex-col items-center justify-center bg-gray-50 dark:bg-[#0B1120] rounded-3xl p-8 border border-gray-100 dark:border-[#1E2D47]">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-4">Result (REM)</span>
            <div className="text-6xl font-black text-blue-600 dark:text-[#00D4B4]">{rem}</div>
          </div>
        </div>
      </div>
    </div>
