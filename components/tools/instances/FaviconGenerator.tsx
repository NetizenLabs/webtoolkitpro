    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {(['square', 'rounded', 'circle'] as const).map(s => (
                <button key={s} onClick={() => setShape(s)} className={`flex-1 py-3 rounded-xl text-xs font-bold capitalize border transition-all duration-300 hover:scale-[1.02] active:scale-95 ${shape === s ? 'bg-[#00D4B4]/10 border-[#00D4B4] text-[#00D4B4] shadow-[0_0_15px_rgba(0,212,180,0.15)]' : 'bg-[#0D1526] border-[#1E2D47] text-gray-400 hover:border-gray-600 hover:text-white'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <button onClick={randomize} className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-purple-900/40 to-fuchsia-900/40 border border-purple-800/50 text-purple-300 rounded-xl text-sm font-bold hover:from-purple-800/50 hover:to-fuchsia-800/50 hover:shadow-[0_0_20px_rgba(168,85,247,0.2)] transition-all duration-300 active:scale-[0.98]">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-3">
              {FAVICON_SIZES.map(size => (
                <button key={size} onClick={() => download(size)} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#00D4B4]/10 to-[#0094FF]/10 border border-[#00D4B4]/30 text-[#00D4B4] rounded-xl text-xs font-bold hover:from-[#00D4B4]/20 hover:to-[#0094FF]/20 hover:shadow-[0_0_15px_rgba(0,212,180,0.2)] transition-all duration-300 hover:-translate-y-0.5 active:scale-95">
                  <Download className="w-3.5 h-3.5" /> {size}×{size}
                </button>
              ))}
            </div>
          </div>
