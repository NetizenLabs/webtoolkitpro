  ...
  const checkDns = async () => {
    ...
    if (isBulkMode) {
      ...
      for (const d of domains) {
        try {
          const res = await fetch(`${PROVIDERS[0].url}?name=${encodeURIComponent(d)}&type=${type}`, {
          ...
          newResults.push({ domain: d, status: 'success', value: answer })
        } catch (e) {
          newResults.push({ domain: d, status: 'error', value: 'Resolution failed' })
        }
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center px-2">
        <BulkModeToggle isBulkMode={isBulkMode} setIsBulkMode={setIsBulkMode} featureName="Bulk DNS Propagation" />
      </div>
            ...
            <div key={i} className="bg-white dark:bg-[#0D1526] border border-gray-100 dark:border-[#1E2D47] rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-black text-gray-900 dark:text-white truncate" title={res.domain}>{res.domain}</span>
                </div>
                <div className={`w-6 h-6 shrink-0 rounded-full flex items-center justify-center ${res.status === 'success' ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}>
                  {res.status === 'success' ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                </div>
              </div>
              <div className="space-y-2">
                <div className="p-3 bg-gray-50 dark:bg-white/5 rounded-xl text-xs font-mono text-blue-600 dark:text-[#00D4B4] border border-black/5 dark:border-white/5 break-all">
